...
export default function TimeEstimateBadge({
  ...
  return (
    ...
    >
      <svg
        className={`h-3.5 w-3.5 shrink-0 ${isDark ? "text-indigo-400" : "text-blue-400"}`}
        fill="none"
        viewBox="0 0 24 24"
        stroke="currentColor"
        strokeWidth={2}
      ...
      {hasEstimate && (
        ...
        >
          Tempo estimado:{" "}
          <span className={`font-bold ${isDark ? "text-indigo-200" : "text-blue-700"}`}>
      ...
      {hasEstimate && (hasQueue || hasRemaining) && (
        <span className={`text-xs ${isDark ? "text-white/15" : "text-gray-300"}`}>
          •
        </span>
      )}
        ...
        >
          {estimate.suaVezEmMin === 0 ? (
            <>
              ...
              <span className={`font-bold ${isDark ? "text-emerald-300" : "text-emerald-600"}`}>
                Próximo na fila!
              </span>
            </>
          ) : (
      ...
      {hasRemaining && (
        ...
        >
          ...
          {remainingMin === 0 ? (
            ...
            <>
              Restante estimado:{" "}
              <span className={`font-bold ${isDark ? "text-emerald-200" : "text-emerald-700"}`}>
